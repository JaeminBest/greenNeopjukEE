...
class SimRunner:
    ...
    def _get_waiting_times(self):
        incoming_roads = ["e_t", "n_t", "w_t", "s_t"]
    ...
    def _get_stats(self):
        halt_N = traci.edge.getLastStepHaltingNumber("n_t")
        halt_S = traci.edge.getLastStepHaltingNumber("s_t")
        halt_E = traci.edge.getLastStepHaltingNumber("e_t")
        halt_W = traci.edge.getLastStepHaltingNumber("w_t")
        intersection_queue = halt_N + halt_S + halt_E + halt_W
        return intersection_queue

    # RETRIEVE THE STATE OF THE INTERSECTION FROM SUMO
    #TODO: adjust size of lane
    def _get_state(self): 
        ...
        for veh_id in traci.vehicle.getIDList():
            ...
            lane_pos = 150 - lane_pos  # inversion of lane pos, so if the car is close to TL, lane_pos = 0
            ...
            if lane_id == "w_t_0" or lane_id == "w_t_1":
                lane_group = 0
            elif lane_id == "n_t_0" or lane_id == "n_t_1":
                lane_group = 2
            elif lane_id == "e_t_0" or lane_id == "e_t_1":
                lane_group = 4
            elif lane_id == "s_t_0" or lane_id == "s_t_1":
                lane_group = 6
